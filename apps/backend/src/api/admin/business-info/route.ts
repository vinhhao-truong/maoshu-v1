...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  const [info] = await (service as any).listBusinessInfos(filter, { take: 1 })
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  const [existing] = await (service as any).listBusinessInfos(filter, { take: 1 })

  let info
  if (existing) {
    ;[info] = await (service as any).updateBusinessInfos([{ id: existing.id, ...body }])
  } else {
    info = await (service as any).createBusinessInfos(body)
  }
